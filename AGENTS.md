...
| Character sheet (read-only) | `client/src/routes/campaigns/[id]/characters/[characterId]/+page.svelte`, `client/src/lib/character-sheet.ts` | `resolveCharacterPlaybook`, `resolveCharacterMoves`, `DEFAULT_LUCK_MAX`, `DEFAULT_HARM_TRACK`. Renders Ratings/Tracks/Moves/Gear/Notes per docs/DESIGN.md; static, non-interactive tracks (tap-to-edit lands in the next roadmap item). Campaign page (`campaigns/[id]/+page.svelte`) lists characters from `db.entities` and links here. |
