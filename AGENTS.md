...
| Top nav & home page (brand link, file-tab section links, role-aware landing page) | `client/src/routes/+layout.svelte`, `client/src/routes/+page.svelte` | The nav renders a `.brand` link (MOWC, back to `/`) plus `.tab` links for "My campaigns"/"Content packs"/"Log in"/"Log out", styled per the docs/DESIGN.md "File tabs" motif: the active route (via `isActive`, comparing `page.url.pathname` from `$app/state`) gets a raised `--surface` background, inactive tabs are `--ink-muted`. Root `/` is a landing page with CTAs that switch on `sessionState.user` (Log in/Register when signed out, My campaigns/Content packs when signed in) rather than a bare `/healthz` status readout; it never repeats a link already in the nav. `Footer.svelte`/`healthState` remain the single place the running app version is displayed. |
