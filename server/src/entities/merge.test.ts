...
describe("mergeCharacterPatch", () => {
  it("takes the patch as the full payload when there is no current row", () => {
    const result = mergeCharacterPatch(undefined, { harm: 1, notes: "x" }, "2026-07-14T10:00:00.000Z", undefined);
  ...
  it("preserves fields the patch does not mention", () => {
    ...
    const result = mergeCharacterPatch(current, { harm: 2 }, "2026-07-14T10:05:00.000Z", "2026-07-14T10:00:00.000Z");
  ...
  it("keeps the current value and flags a conflict when the op is older", () => {
    ...
    const result = mergeCharacterPatch(current, { harm: 9 }, "2026-07-14T10:00:00.000Z", "2026-07-14T10:05:00.000Z");
  ...
  it("does not flag a conflict when the patch value already matches", () => {
    ...
    const result = mergeCharacterPatch(current, { harm: 3 }, "2026-07-14T10:00:00.000Z", "2026-07-14T10:05:00.000Z");
